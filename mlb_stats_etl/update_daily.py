...
def main():
    ...
    ap.add_argument("--lookback-days", type=int, default=3, help="Days back to refresh (captures retroactive changes)")
    ap.add_argument("--lookahead-days", type=int, default=1, help="Days forward to include scheduled and in-progress games")
    ...
    # Reference snapshots
    ref = fetch_reference_frames(client, season=today.year)

    # Games for the window
    sched_df = fetch_schedule_by_dates(client, start.strftime("%Y-%m-%d"), end.strftime("%Y-%m-%d"))
    # Identify games we can skip: already-final and in state
    ...
    g, l, t, p, plays, pitches = (pd.DataFrame(),)*6
    if to_fetch:
        g, l, t, p, plays, pitches = fetch_game_feeds(client, to_fetch)

    # Determine which games are final now (from the newly fetched games)
    newly_final = set()
    if not g.empty:
        # Use codedGameState == 'F' to detect final
    ...
    # Leaders & stats for current season-to-date
    season = today.year
